---
title: "10. Unsupervised Learning II"
output:
  html_document:
    theme: cosmo
    highlight: zenburn
...
The kmeans function has an option for returning the results as a data frame:

```{r}
df <- dsst_kmeans(anno, docs, doc_var = "label", n_dims = 2, n_clusters = 5L,
                  output_type="df")
df
```

This allows us to use the data to build other visualisations:
...
Hopefully you are starting to see the possibilites of things that we can do
...
df <- anno %>%
  ...
  dsst_kmeans(invert = TRUE, n_clusters = 5L, n_dims = 15L, output_type="df")
df
```
